...
def compute_ndrope_cis(freqs: torch.Tensor, position: torch.Tensor, emb_dim: int, num_heads: int,
                       theta: float = 100.0, device=None, dtype=torch.float32,yarn_factor: float = 1.0, 
                       yarn_cutoff: float = 0.6, 
                       yarn_sharpness: float = 8.0, 
                       yarn_power: float = 1.0):
    ...
    if yarn_factor is not None and float(yarn_factor) > 1.0:
        r = torch.arange(S, device=device, dtype=dtype) / max(S - 1, 1) 
        m = torch.sigmoid(yarn_sharpness * (r - yarn_cutoff))   
        if yarn_power != 1.0:
            m = m ** yarn_power
        mag = mag / (float(yarn_factor) ** m)
    # --------------------------------------------------------------------
...
class Attention(nn.Module):
    ...
    def forward(self, x, freqs_cis=None):
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale * self._yarn_inv_t
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                ...
                yarn_sharpness: float = 8.0,
                yarn_power: float = 1.0,
                enable_yarn_logit: bool = True,
                logit_clip: tuple = (0.5, 9.0)):
        super().__init__()
        self.attention_type = attention_type
        self.depth = depth
        ...
        self.yarn_cutoff = yarn_cutoff
        self.yarn_sharpness = yarn_sharpness
        self.yarn_power = yarn_power
        self.enable_yarn_logit = enable_yarn_logit
        ...
        # ===== YaRN Logit Scaling Setup =====
        self.inv_t = 1.0
        if self.enable_yarn_logit and self.yarn_factor > 1.0:
             inv_sqrt_t = 0.1 * math.log(self.yarn_factor) + 1.0
             self.inv_t = inv_sqrt_t * inv_sqrt_t
             lo, hi = self.logit_clip
             self.inv_t = float(max(lo, min(hi, self.inv_t)))
        
        # Inject inv_t into blocks
        for blk in self.blocks:
            if hasattr(blk, 'attn'):
                blk.attn._yarn_inv_t = self.inv_t
    ...
    def forward_features(self, x):
        ...
        if self.use_ndrope:
            ...
            freqs_cis_spatial = compute_ndrope_cis(
                self.freqs_spatial, pos_xy, self._head_dim, self.blocks[0].attn.num_heads, 
                self.theta_spatial, device=x.device, dtype=x.dtype,
                yarn_factor=self.yarn_factor, 
                yarn_cutoff=self.yarn_cutoff, 
                yarn_sharpness=self.yarn_sharpness, 
                yarn_power=self.yarn_power
            )
            
            freqs_cis_temporal = compute_ndrope_cis(
                self.freqs_temporal, pos_t, self._head_dim, self.blocks[0].attn.num_heads, 
                self.theta_temporal, device=x.device, dtype=x.dtype,
                yarn_factor=1.0,
            )
        else:
...
def get_yarn_params(cfg, kwargs):
    """
    Priority: kwargs > cfg.MODEL > Default
    """
    # Helper lambda to check config existance
    get_cfg = lambda name, default: getattr(cfg.MODEL, name, default) if (cfg and hasattr(cfg, 'MODEL')) else default

    yarn_factor = kwargs.pop('yarn_factor', get_cfg('YARN_FACTOR', 1.0))
    yarn_cutoff = kwargs.pop('yarn_cutoff', get_cfg('YARN_CUTOFF', 0.6))
    yarn_sharpness = kwargs.pop('yarn_sharpness', get_cfg('YARN_SHARPNESS', 8.0))
    yarn_power = kwargs.pop('yarn_power', get_cfg('YARN_POWER', 1.0))
    enable_yarn_logit = kwargs.pop('enable_yarn_logit', get_cfg('ENABLE_YARN_LOGIT', True))
    
    return {
        'yarn_factor': yarn_factor,
        'yarn_cutoff': yarn_cutoff,
        'yarn_sharpness': yarn_sharpness,
        'yarn_power': yarn_power,
        'enable_yarn_logit': enable_yarn_logit
    }
